...
設定の **アカウント (계정)** 画面に、接続・切り替え・削除と残りの上限をまとめて表示します。

- 接続済み Codex・AGY・Claude アカウントと管理型 Kimi ログインの各行に、コンパクトな残量チップを表示
- 一つの照会に失敗しても他のアカウント行と利用量を維持
- Codex が返す実期間から 5 時間・週間・月間の上限を表示し、GPT-5.3-Codex のモデル専用上限は表示しません
- 残量 30% 以下は黄色、10% 以下は赤色
...
|---|---|
| クリック | あいさつ |
| ダブルクリック | ジャンプして現在の Codex 使用量を吹き出しで表示 |
| ドラッグ | ペットを移動 |
...
- **Codex**: プロフィールごとに認証情報を保存します。Codex Desktop の実行中にアカウントを切り替えると、CodePet が終了を要求し、実際の終了を確認してから認証情報を置き換え、自動的に再起動します。Codex を手動で終了する必要はありません。**Codex 上限自動切り替え (ローカルエンジン) (Codex 한도 자동 전환 (로컬 엔진))** は 組み込み OpenCodex エンジン (`127.0.0.1`) で新しい CLI 接続の認証情報を適用し、上限到達後にアカウントをローテーションします。`~/.codex/config.toml` の `# codepet-codex-provider` ブロックを管理します。すでに開いている CLI セッションは、選択したアカウントを使うために起動し直す場合があります。
...
強制終了後に Codex が接続できない場合は CodePet を一度起動して古いエンジンマーカーを整理してください。必要なら `~/.codex/config.toml` から `# codepet-codex-provider` ブロックを削除できます。
