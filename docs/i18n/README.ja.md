...
- **Codex**: プロフィールごとに認証情報を保存します。Codex Desktop の実行中にアカウントを切り替えると、CodePet が終了を要求し、実際の終了を確認してから認証情報を置き換え、自動的に再起動します。Codex を手動で終了する必要はありません。**Codex 上限自動切り替え (ローカルプロキシ) (Codex 한도 자동 전환 (로컬 프록시))** は `127.0.0.1` 経由で新しい CLI 接続の認証情報を適用し、上限到達後にアカウントをローテーションします。`~/.codex/config.toml` の `# codepet-codex-proxy` ブロックを管理します。すでに開いている CLI セッションは、選択したアカウントを使うために起動し直す場合があります。
