...
|---|---|---|---|
| 通过动作和气泡显示响应编写、文件修改、命令、测试、等待批准与完成。 | 为每个已连接的 Codex、AGY 和 Claude 账户显示独立额度卡片。 | 跨 provider 按开始顺序追踪最多 5 个任务。 | 加载 Codex 宠物和自定义精灵图，并保存大小、位置与移动偏好。 |
...
- 为每个已连接的 Codex、AGY 和 Claude 账户显示独立卡片
- 单个账户查询失败不会隐藏其他卡片
